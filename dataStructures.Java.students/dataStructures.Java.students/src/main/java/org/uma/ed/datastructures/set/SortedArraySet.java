...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public static <T> SortedArraySet<T> copyOf(SortedSet<T> that) {
    SortedArraySet<T> copySet = new SortedArraySet<T>(that.comparator());
    for (T element : that){
      copySet.insert(element);
    }
    return copySet;
  }
  ...
  public void insert(T element) {
    ...
    if (!finder.found) {
      ensureCapacity();
      System.arraycopy(elements, finder.index, elements, finder.index + 1, size - finder.index);
  ...
  public void delete(T element) {
    Finder finder = new Finder(element);
    if (finder.found) {
      int numMoved = size - finder.index - 1;
      if (numMoved > 0) {
        System.arraycopy(elements, finder.index + 1, elements, finder.index, numMoved);
      }
      elements[--size] = null;
    }
  }
  ...
  public static <T> SortedArraySet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    SortedArraySet<T> result = new SortedArraySet<>(comparator);

    for (T element : sortedSet1) {
      result.insert(element);
    }
    for (T element : sortedSet2) {
      result.insert(element);
    }
  ...
  public static <T> SortedArraySet<T> intersection(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    for (T element : sortedSet1) {
      if (sortedSet2.contains(element)) {
        result.insert(element);
      }
    }
    return result;
