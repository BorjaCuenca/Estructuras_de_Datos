...
public class HashBag<T> extends AbstractBag<T> implements Bag<T> {
  ...
  public void delete(T element) {
    Pair<T> key = hashTable.search(Pair.withElement(element));
    if (key != null){
      int occurrences = key.occurrences();
      hashTable.delete(key);
      if (occurrences > 1) {
        hashTable.insert(Pair.of(element,occurrences-1));
      }
    }
  }
  ...
  public void insert(T element) {
    Pair<T> key = hashTable.search(Pair.withElement(element));
    if (key != null) {
      int occurences = key.occurrences();
      hashTable.delete(key);
      hashTable.insert(Pair.of(element, occurences+1));
    }
    else {
      hashTable.insert(Pair.withElement(element));
    }
  }
