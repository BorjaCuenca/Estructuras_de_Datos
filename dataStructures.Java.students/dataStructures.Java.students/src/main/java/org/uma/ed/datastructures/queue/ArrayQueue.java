...
public class ArrayQueue<T> extends AbstractQueue<T> implements Queue<T> {
  ...
  public static <T> ArrayQueue<T> copyOf(ArrayQueue<T> that) {
    ArrayQueue copyQ = new ArrayQueue(that.elements.length);
    copyQ.size = that.size;
    copyQ.first = that.first;
    copyQ.last = that.first;
    for (int c=0, i=that.first; c < that.size(); c++){
      copyQ.elements[i] = that.elements[i];
      i = that.advance(i);
    }
    return copyQ;
  }
