...
public class SortedLinkedSet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public static <T> SortedLinkedSet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    SortedLinkedSetBuilder<T> builder = new SortedLinkedSetBuilder<>(comparator);

    Iterator<T> iter1 = sortedSet1.iterator();
    Iterator<T> iter2 = sortedSet2.iterator();

    T element1 = nextOrNull(iter1);
    T element2 = nextOrNull(iter2);

    while (element1 != null && element2 != null) {
      int cmp = comparator.compare(element1, element2);

      if (cmp < 0) {
        builder.append(element1);  // Añadimos el elemento de sortedSet1
        element1 = nextOrNull(iter1);
      } else if (cmp > 0) {
        builder.append(element2);  // Añadimos el elemento de sortedSet2
        element2 = nextOrNull(iter2);
      } else {
        builder.append(element1);  // Si son iguales, añadimos solo uno de ellos
        element1 = nextOrNull(iter1);
        element2 = nextOrNull(iter2);
      }
    }

    // Añadimos los elementos restantes de sortedSet1 (si los hay)
    while (element1 != null) {
      builder.append(element1);
      element1 = nextOrNull(iter1);
    }

    // Añadimos los elementos restantes de sortedSet2 (si los hay)
    while (element2 != null) {
  ...
  public static <T> SortedLinkedSet<T> intersection(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while (element1 != null && element2 != null) {
      ...
      if (cmp == 0) {
        builder.append(element1);  // Añadimos el elemento
        element1 = nextOrNull(iter1);
      }
    }
