...
public class HashDictionary<K, V> extends AbstractDictionary<K, V> implements Dictionary<K, V> {
  ...
  public void insert(Entry<K, V> entry) {
    Entry<K,V> searchedKey = hashTable.search(entry);
    if (searchedKey != null) {
      searchedKey = entry;
    }
    else {
  ...
  public V valueOf(K key) {
    Entry<K,V> searchedKey = hashTable.search(Entry.withKey(key));
    return (searchedKey == null ? null : searchedKey.value());
  }
